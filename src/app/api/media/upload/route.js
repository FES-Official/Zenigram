...
function validateNewUpload(body) {
  ...
  if (!fileName || !ALLOWED_MEDIA_TYPES.has(contentType)) {
  ...
  try {
    ...
    if (action === "createMultipart") {
      ...
      if (!contentType.startsWith("video/")) {
        return jsonError("Multipart upload is reserved for videos", 400);
      }
      const partSize = Math.max(
        MIN_PART_SIZE,
        Math.min(DEFAULT_PART_SIZE, Math.ceil(size / 10000)),
      );
      ...
      return jsonOk({
        ...
        objectUrl: getPublicS3Url(key),
      });
    }

    const key = normalizeString(body.key);
    const uploadId = normalizeString(body.uploadId);
    if (!key || !uploadId || !isOwnedMediaKey(key, session.user.id)) {
    ...
    if (action === "completeMultipart") {
      const parts = Array.isArray(body.parts)
        ...
        : [];
      if (!parts.length) return jsonError("Uploaded parts are required", 400);
      await s3.send(
