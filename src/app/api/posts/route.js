...
import { createPost, getUserById, listFeedPosts } from "@/app/lib/socialStore";

const MAX_IMAGES_PER_POST = 10;
const PRESENTATIONS = new Set(["single", "carousel", "grid"]);
const CAROUSEL_STYLES = new Set([
  "classic",
  "fade",
  "stack",
  "filmstrip",
...
export async function POST(req) {
  try {
    ...
    const presentation = PRESENTATIONS.has(body.presentation)
      ? body.presentation
      : "single";
    const carouselStyle = CAROUSEL_STYLES.has(body.carouselStyle)
      ? body.carouselStyle
      : "classic";
    const gridLayout = GRID_LAYOUTS.has(body.gridLayout)
      ? body.gridLayout
      : "";
    const aspectRatio = ASPECT_RATIOS.has(body.aspectRatio)
      ? body.aspectRatio
      : "square";
    const requestedMedia = Array.isArray(body.mediaItems)
      ? body.mediaItems.slice(0, MAX_IMAGES_PER_POST)
      : [];

    if (!caption || caption.length > 2200) {
      return jsonError("Caption required", 400);
    }
    ...
    if (verified.some((item) => !item)) {
      return jsonError("One or more S3 objects could not be verified", 400);
    }
...
export async function GET() {
  try {
    ...
    const post = await listFeedPosts(session.user.id);
