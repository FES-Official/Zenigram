  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError(requestError.message || "Unable to load saved content");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <main className="min-h-screen bg-linear-to-br from-zinc-950 via-red-950 to-black text-white">
      ...
      <section className="mx-auto min-h-screen w-full max-w-5xl px-4 pb-24 pt-8 md:ml-20 md:pb-10">
        <Link href=".." className="text-xs font-semibold text-red-300 hover:text-red-200">← Back to profile</Link>
        ...
        {loading ? (
          <div className="mt-6 text-sm text-white/40">Loading your saved library…</div>
        ) : tab === "clips" ? (
          clips.length ? (
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
              {clips.map((clip) => (
                <Link key={clip._id} href={`/clips?clip=${encodeURIComponent(clip._id)}`} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-black/40">
                  {clip.mediaType === "video" ? <video src={clip.mediaUrl} muted playsInline preload="metadata" className="aspect-9/16 w-full object-cover transition group-hover:scale-105" /> : <Image src={clip.mediaUrl} alt={clip.caption || "Saved clip"} width={360} height={640} unoptimized className="aspect-9/16 w-full object-cover transition group-hover:scale-105" />}
              ...
              {posts.map((post) => {
                ...
                return (
                  <Link key={post._id} href={`/profile/${encodeURIComponent(post.user?.username || "")}`} className="overflow-hidden rounded-2xl border border-white/10 bg-black/30">
                    {media.type === "video" ? <video src={media.url} muted playsInline preload="metadata" className="aspect-square w-full object-cover" /> : <Image src={media.url} alt={post.caption || "Saved post"} width={600} height={600} unoptimized className="aspect-square w-full object-cover" />}
